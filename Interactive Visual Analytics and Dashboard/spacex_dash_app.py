...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        fig = px.pie(values=values, names=labels, title=f'Success Rate for {entered_site}', labels={'class': 'Success'})
...
def get_scatter_chart(entered_site, entered_load):
    ...
    filtered_df = spacex_df[(spacex_df['Launch Site'] == entered_site) &
                            (spacex_df['Payload Mass (kg)'].between(entered_load[0], entered_load[1]))]
